...
class TestConvertRpmLicenseToSpdx(unittest.TestCase):
    """
    Unit tests for convert_rpm_license_to_spdx function.
    """

    @patch("subprocess.run")
    def test_convert_gpl_license(self, mock_run):
        """Test converting GPL license."""
        mock_run.return_value = MagicMock(stdout="GPL-3.0-or-later\n", returncode=0)
        result = convert_rpm_license_to_spdx("GPL-3.0-or-later")
    ...
    @patch("subprocess.run")
    def test_convert_lgpl_license(self, mock_run):
        """Test converting LGPL license."""
        mock_run.return_value = MagicMock(stdout="LGPL-2.1-or-later\n", returncode=0)
        result = convert_rpm_license_to_spdx("LGPL-2.1-or-later")
        self.assertEqual(result, "LGPL-2.1-or-later")

    @patch("subprocess.run")
    def test_empty_license(self, mock_run):
        """Test with empty license string."""
        result = convert_rpm_license_to_spdx("")
        self.assertEqual(result, "NOASSERTION")
        mock_run.assert_not_called()

    @patch("subprocess.run")
    def test_none_license(self, mock_run):
        """Test with None license."""
        result = convert_rpm_license_to_spdx(None)
        self.assertEqual(result, "NOASSERTION")
        mock_run.assert_not_called()
    ...
    def test_conversion_failure(self, mock_run):
        ...
        mock_run.side_effect = subprocess.CalledProcessError(1, "license-fedora2spdx")
        result = convert_rpm_license_to_spdx("InvalidLicense")
        self.assertEqual(result, "NOASSERTION")

    @patch("subprocess.run")
    def test_command_not_found(self, mock_run):
        """Test handling when license-fedora2spdx is not found."""
        mock_run.side_effect = FileNotFoundError("license-fedora2spdx not found")
        result = convert_rpm_license_to_spdx("GPL-2.0")
        self.assertEqual(result, "NOASSERTION")

    @patch("subprocess.run")
    def test_timeout(self, mock_run):
        """Test handling of command timeout."""
        mock_run.side_effect = subprocess.TimeoutExpired("license-fedora2spdx", 5)
        result = convert_rpm_license_to_spdx("GPL-2.0")
        self.assertEqual(result, "NOASSERTION")
    ...
    def test_empty_output(self, mock_run):
        ...
        result = convert_rpm_license_to_spdx("UnknownLicense")
        self.assertEqual(result, "NOASSERTION")
...
class TestAttachBuildrootPackages(unittest.TestCase):
    ...
    @patch("python_scripts.merge_syft_sbom.convert_rpm_license_to_spdx")
