...
class PulpClient:
    ...
    def find_content_by_build_id(self, build_id):
        url = self.url(f"api/v3/content/?pulp_label_select=build_id~{build_id}&fields=artifact")
        return requests.get(url, **self.request_params)
...
def upload_rpm(client, rpm_repository_prn, rpm, labels):
    ...
    client.create_rpm_content(rpm_repository_prn, rpm, labels)
    
def upload_log(client, file_repository_prn, log, build_id, labels):
    logging.info(f"Uploading log file: {log}")
    client.create_file_content(file_repository_prn, log, build_id, labels)

    
def upload_rpms_logs(rpm_path, args, client, arch):
...
def collect_results(client):
    ...
    with open("results.json", "w") as outfile:
        logging.info(f"Writing Quay URL results to results.json")
...
def upload_sbom(client, args):
    ...
    sbom_file = "/var/workdir/results/oras-staging/sbom-merged.json"
    logging.info(f"Uploading sbom file: {sbom_file}")
    content_upload_response = client.create_file_content(file_repository_prn, sbom_file, args.build_id,labels)
...
if __name__ == "__main__":
    ...
    # Create an rpm distribution
    client.create_rpm_distribution(repository_name, rpm_repository_prn, basepath=repository_name)
    ...
    # Create an file distribution
    client.create_file_distribution(repository_name, file_repository_prn, basepath=repository_name)
    ...
    for arch in archs:
        ...
        upload_rpms_logs(current_path, args, client, arch)
